...
def main():
    ...
    processor = Sam3Processor(lora_model, device=device, confidence_threshold=0.5)
    
    epoch_weights = sorted(glob.glob(os.path.join(WEIGHTS_DIR, "lora_weights_epoch_*.pt")))
    
    for weight_path in epoch_weights:
        # Extract epoch number
        epoch_str = os.path.basename(weight_path).replace("lora_weights_epoch_", "").replace(".pt", "")
        epoch = int(epoch_str)
