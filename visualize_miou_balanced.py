...
VIS_DIR = "/teamspace/studios/this_studio/SAM3_Testing/visualizations_balanced"
...
def build_model(with_lora=False):
    ...
    if with_lora:
        ...
        # Using the best_lora_weights.pt (Epoch 3)
...
def run_inference(model, processor, valid_samples, device):
    results = {}
...
def main():
    ...
    # Categorize all data by prompt type to ensure balanced visualization
    categorized_data = {
        "damaged building": [],
        "intact building": [],
        "road damage": [],
    ...
    print("Parsing dataset to find valid samples for each prompt type...")
    for item in data:
        image_rel_path = item['post_image_path'].replace('\\', '/')
    ...
    # Select samples (aiming for 10 total, balanced)
    num_total = 10
    ...
    for cat in categories:
        random.shuffle(categorized_data[cat])
        
    # Pick iteratively round-robin
    idx = 0
    while len(valid_samples) < num_total:
        cat = categories[idx % len(categories)]
        if len(categorized_data[cat]) > 0:
            valid_samples.append(categorized_data[cat].pop(0))
        idx += 1
        
        # Break if all categories are empty
        if all(len(categorized_data[c]) == 0 for c in categories):
            break
            
    print(f"Selected {len(valid_samples)} valid samples for visualization:")
    for cat in categories:
        count = sum(1 for s in valid_samples if s["text_prompt"] == cat)
    ...
    base_results = run_inference(model_base, processor_base, valid_samples, device)
    ...
    lora_results = run_inference(model_lora, processor_lora, valid_samples, device)
    ...
    for i, sample in enumerate(valid_samples):
        ...
        # Save
        safe_name = os.path.basename(rel_path).replace('.png', f'_{i}.jpg')
